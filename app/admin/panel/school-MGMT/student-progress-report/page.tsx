...
export default function StudentProgressReportPage() {
  ...
  {/* Left Section: Avatar + Info + Settings */}
  <div className="flex flex-col sm:flex-row gap-4 sm:items-center w-full sm:w-auto">
    <div className="flex gap-4 sm:gap-6 items-start sm:items-center w-full">
      {/* Avatar */}
      <Avatar className="size-24 sm:size-28 mx-auto sm:mx-0">
      ...
      <div className="flex flex-col gap-2 text-center sm:text-left w-full">
        <div className="flex items-center justify-between w-full">
          <h1 className="font-semibold text-lg sm:text-xl tracking-wide">Shlok Ageda</h1>

          {/* Settings Icon (aligned to top-right in small widths) */}
          <button
            onClick={() => setOpenModal("confidential")}
            className="rounded-full p-1.5 border border-[#E5E7EB] bg-[#F9FAFB] cursor-pointer sm:ml-4"
          >
        ...
        <div className="flex flex-wrap justify-start sm:justify-start gap-2">
          <div className="bg-[#FF3366] rounded-l-full text-xs font-light px-3 py-1.5 text-white">Class 8A</div>
          <div className="bg-[#FF3366] rounded-r-full text-xs font-light px-3 py-1.5 text-white">Group A</div>
        </div>
      </div>
    </div>
  </div>
  ...
  <div className="w-full sm:w-auto space-y-1 mt-4 sm:mt-0 text-sm">
    <p className="font-medium">Gender: <span className="font-light">Male</span></p>
    <p className="font-medium">DOB: <span className="font-light">15 June 2015</span></p>
    <p className="font-medium">Email: <span className="font-light">example@gmail.com</span></p>
    <p className="font-medium">Contact: <span className="font-light">+91 1234567890</span></p>
    <p className="font-medium">City: <span className="font-light">Mumbai</span></p>
    <p className="font-medium">State: <span className="font-light">Maharashtra</span></p>
  </div>
</div>
              ...
              <div className="mt-6">
                ...
                <div className="flex flex-wrap gap-2 items-center justify-start text-sm mt-3">
                  <p className="bg-[#F3F4F6] px-3 py-1.5 rounded-full border border-[#B0B0B0]">
                    Academics
                  </p>
                  <p className="bg-[#F3F4F6] px-3 py-1.5 rounded-full border border-[#B0B0B0]">
          ...
          <MaxWidthWrapper className="bg-[#F1F1F2] p-4 rounded-2xl border border-[#E5E7EB]">
            <div>
              <h1 className="text-[#FF3366] font-semibold text-xl">
                Active Courses
              </h1>
  ...
  return (
    <div>
      ...
      <div className="flex flex-wrap lg:flex-nowrap gap-4 w-full mb-4 h-[31rem]">
        <div className="w-full lg:w-[65%] bg-white p-4 rounded-2xl">
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6">
            <p className="font-semibold text-lg mb-3 text-[#FF3366]">
              Overall Progress
            </p>
          </div>
          <div className="flex items-start flex-col-reverse gap-4 sm:flex-row justify-between px-7 mb-5">
            <div className="flex flex-col flex-wrap gap-x-4 gap-y-2 mb-4 text-xs text-nowrap">
              <div className="flex items-center">
                ...
                <p className="text-[#626262] text-sm font-light">
      ...
      <div className="relative">
        <div
          className="flex items-center gap-2.5 text-sm border border-[#E5E7EB] text-black bg-[#F9FAFB] px-3 py-2 rounded-xl cursor-pointer"
        ...
        {showDropdown && (
          <div className="absolute z-10 bg-white border border-gray-200 shadow-md mt-1 rounded-xl max-h-60 overflow-y-auto no-scrollbar w-full">
            {months.map((month, index) => (
              <div
                key={month}
                onClick={() => handleMonthSelect(index)}
                className={`px-4 py-2 text-sm hover:bg-gray-100 cursor-pointer ${
                  index === selectedMonth ? 'bg-gray-100 font-medium' : ''
                }`}
              >
                {month}
              </div>
      ...
      {/* Year Control */}
        <ArrowControl
        leftOnClick={decreaseYear}
        RightOnClick={increaseYear}
        text={year.toString()}
      />
    </div>
          </div>
            ...
            >
              ...
              {lineChartData.labels.map((label, i) => (
                <text
                  key={label}
                  x={
                    35 + i * (chartWidth / (lineChartData.labels.length - 0.6))
                  }
                  y={chartHeight + 25}
                  fontSize="7"
                  fill={"#626262"}
        ...
        <div className="pt-1 mb-10 h-full rounded-2xl w-full lg:w-[35%] p-4 bg-white">
          <h3 className="text-base mt-4 font-semibold text-[#FF3366]">
            Parameter 1
          </h3>
      ...
      <div className="flex flex-wrap lg:flex-nowrap gap-4 relative">
        <div className="w-full lg:w-[65%]">
          ...
          <div className="rounded-2xl flex-1 mt-4 overflow-hidden">
            <table className="w-full rounded-2xl text-sm">
              <thead className="bg-[#E5ECFF]">
                <tr>
                  {[
                    ...
                    <th
                      key={index}
                      className="p-4 text-left font-medium tracking-wide"
                    >
                      {header}
                    </th>
                  ))}
              ...
              <tbody>
                {testResultsData.map((row, i) => (
                  <tr
                    key={i}
                    className="odd:bg-[#F7F9FF] even:bg-[#FFFFFF] text-[#777777] "
                  >
                    ...
                    <td className="p-4 odd:text-[#8DD9B3] even:text-[#FF3366] font-medium">
                      {row.result}
                    </td>
                  </tr>
				  
                ))}
                <tr className="bg-[#E5ECFF]/60 ">
                  ...
                  <td className="p-2.5 odd:text-[#E5ECFF4D] even:text-[#FFFFFF] font-medium">
                    asdf
                  </td>
                </tr>
              </tbody>
        ...
        <div className="w-full lg:w-[35%] p-4 rounded-2xl bg-white">
          <div className="bg-[#FBD2D9] p-4 rounded-2xl text-[#893544]">
            <h1 className="font-semibold tracking-wide">Parameter 4</h1>
            <p className="text-xs my-4">Overall Progress</p>
            <div className="mt-2 text-sm relative">
              <div className="absolute bottom-14 left-[85%]">
                <Image
                  ...
                  height={86}
                  alt="Maths Sign"
                  className="w-10 aspect-square h-10"
                />
              </div>
              <span className="font-semibold">4/5</span>
              <div className="flex gap-2 items-center -mt-1.5">
