    ...
    <><NamingBar name="Manage Approvals" />
     <div className="bg-[#EEEEEE] p-4">
      ...
      <MaxWidthWrapper className="bg-white rounded-2xl my-10 py-4">
        ...
        />
		<RejectChoiceModal isOpen={openModal === "rejectchoice"}
						onClose={() => setOpenModal(null)}/>
        <SearchFilterBar />
        ...
        {activeTab === "Schools" && (
          <div className="grid gap-3 sm:gap-4 grid-cols-1 lg:grid-cols-2">
            {Array.from({ length: 6 }).map((_, i) => (
              <SchoolCard key={i} setOpenModal={setOpenModal} />
            ))}
          </div>
        )}
        ...
        {activeTab === "Teachers" && (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {Array.from({ length: 12 }).map((_, i) => (
              <TeacherRectangle key={i} setOpenModal={setOpenModal}/>
            ))}
          </div>
        )}
        ...
        {activeTab === "Students" && (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {Array.from({ length: 12 }).map((_, i) => (
              <StudentRectangle key={i} setOpenModal={setOpenModal} /> // replace with StudentRectangle if different
            ))}
          </div>
        )}
      </MaxWidthWrapper>
...
function SchoolCard({ setOpenModal }: { setOpenModal: (value: string) => void }) {
    ...
    <Card className="shadow-none bg-[#F9FAFB] flex flex-col sm:flex-row gap-3 sm:gap-4 p-3 sm:p-4">
      ...
      <div className="flex flex-col justify-between gap-2 flex-1">
        ...
        <div className="flex gap-4 justify-end items-center mt-auto">
          <Button
            variant="secondary"
            className="bg-[#FF33661A] text-[#FF3366] rounded-full px-6"
          onClick={() => setOpenModal("rejectchoice")} >
            Reject
          </Button>
          <Button
            className="rounded-full px-6"
            onClick={() => setOpenModal("confirmMessage")}
          >
            Approve
          </Button>
        </div>
...
function TeacherRectangle({ setOpenModal }: { setOpenModal: (value: string) => void }) {
    ...
    <div className="bg-[#F3F4F6] rounded-2xl px-2 py-1.5 border border-[#B0B0B0] flex items-center gap-1">
      ...
      <div className="ml-1 flex items-center justify-between w-full">
        ...
        <div className="flex gap-2 items-center">
          ...
          <Button className="rounded-full text-[#FF3366] bg-[#FF33661A] px-2" onClick={() => setOpenModal("rejectchoice")} >>
            Reject
          </Button>
        </div>
      </div>
...
function StudentRectangle({ setOpenModal }: { setOpenModal: (value: string) => void }) {
