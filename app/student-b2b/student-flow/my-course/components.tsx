...
export const CourseCard: React.FC<CourseCardProps> = ({ course }) => {
	...
	if (course.isKnowledgeBox) {
		return (
			<div
				...
				onClick={() => router.push("/student-b2b/student-flow/my-class")}
				className="bg-[#F9FAFB] border-[#E5E7EB] border rounded-2xl p-3 flex flex-col h-full  duration-200">
	...
	return (
		<div
		onClick={() => router.push("/student-b2b/student-flow/single-course-detail")} className="bg-[#F9FAFB] border-[#E5E7EB] border rounded-2xl p-2 flex flex-col h-full duration-200">
			...
			<div className="flex flex-col flex-grow mt-1"> {/* Adjusted margin */}
				{course.category && (
					<p className="text-xs font-semibold text-[#FF3366] uppercase mb-1">{course.category}</p>
				)}
				<h3 className="text-lg sm:text-xl font-bold text-black my-1  leading-tight">{course.name}</h3>
