...
export default function GradeAndContent({ searchParams }: { searchParams: Promise<{ step?: string }> }) {
	...
	const [activeTab, setActiveTab] = useState<'Academic Courses' | 'Foundation & Skills Courses'>(
		currentTestStep === 1 ? 'Academic Courses' : 'Foundation & Skills Courses'
	)
	...
	return (
		<div className="bg-[#EEEEEE] h-full min-h-screen">
			...
			<MaxWidthWrapper className="py-10">
				...
				{activeTab === 'Foundation & Skills Courses' && <FoundationCard />}
			</MaxWidthWrapper>
		</div>
	)
}
...
Lorem Ipsum is simply dummy text of the printing and typesetting industry.`,
	...
	return (
		<div className="flex  flex-wrap gap-6 ">
			{cardsData.map(card => (
				<div key={card.id} className="bg-white rounded-xl  w-full max-w-[450px] overflow-hidden  shadow-md">
					<Image src={card.image} alt="Grade Card" width={572} height={572} className="w-full max-h-56 object-cover" />
					<div className="p-4 flex text-center flex-col flex-grow">
						<h3 className="text-[1.3rem] text-center mb-2 font-medium">{card.title}</h3>
						<p className="text-[#2E2E2E] text-center mx-auto  max-w-[39ch] text-sm tracking-wide leading-6">{card.description}</p>
						<Link href={'/b2c-phase-3/grade-and-content'} className="w-full mt-4">
							<button className="bg-[#3366ff] rounded-full w-full px-3 py-2   text-white">{card.buttonLabel}</button>
						</Link>
					</div>
				</div>
			))}
