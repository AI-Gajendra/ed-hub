...
const RequestDiagnostic : React.FC<PopupProp> = ({
    isOpen,
    onClose,
}) => {
  ...
  return (
     <BaseModal isOpen={isOpen} onClose={onClose} maxWidth="max-w-md">
      <div className="bg-white p-8 rounded-3xl w-[90%]">
        <h2 className="text-xl font-semibold text-center mb-6">Request Diagnostic Test</h2>

        {/* Select Assessment */}
        <div className="mb-4">
          <label className="block text-sm font-medium mb-1">Select Diagnostic Test</label>
          <div className="relative">
            ...
            >
             <option value="Diagnostic Test1">Select Option</option>
              <option value="Diagnostic Test1">Diagnostic Test1</option>
              <option value="Diagnostic Test2">Diagnostic Test2</option>
            </select>
            <IoIosArrowDown className="absolute right-4 top-1/2 transform -translate-y-1/2 text-[#6b7280" />
          </div>
        </div>
...
export default RequestDiagnostic;