		...
		<motion.div
			...
			<div className="z-40 min-h-80 w-full absolute top-0 translate-y-[-100%] flex justify-center text-white">
				<div className="min-h-24 max-w-6xl bg-[#f9346d] w-full p-4 rounded-2xl flex px-16 items-center justify-between relative top-40 overflow-hidden shadow-xl">
					<svg className="absolute top-0 left-0  w-28 h-28 sm:w-40 sm:h-40" viewBox="0 0 100 100">
						<motion.circle
							cx="30"
							cy="20"
							r="45"
					...
					<svg className="absolute top-0 right-0 w-28 h-28 sm:w-40 sm:h-40" viewBox="0 0 100 100">
						<motion.circle
							cx="90"
							cy="30"
							r="45"
					...
					<div className="flex flex-col items-center justify-center w-full">
						...
						<motion.div
							...
							className="flex justify-center sm:flex-row w-full md:max-w-md mx-auto rounded-full p-1 bg-white shadow-lg xs:max-w-xs sm:max-w-md">
							<input
								type="email"
								placeholder="Email Address"
								className="flex-grow sm:py-3 px-3 py-2 sm:px-6 text-gray-800 outline-none text-sm sm:text-base min-w-0 rounded-l-full sm:rounded-l-full"
							/>
							<motion.button
								whileHover={{ scale: 1.05 }}
								whileTap={{ scale: 0.95 }}
								className="bg-[#FFCC00] font-bold text-white rounded-full p-3 sm:py-3 sm:px-8 transition-all duration-300 hover:bg-[#262623] cursor-pointer text-sm sm:text-base whitespace-nowrap flex items-center justify-center gap-1 flex-shrink-0">
								<FiSend className="block  sm:hidden text-lg" /> {/* Icon on mobile */}
								<span className="hidden sm:block">Send</span> {/* Text on larger screens */}
							</motion.button>
						</motion.div>
					</div>
					...
					<motion.div variants={staggerChildren} className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 ">
						...
						<motion.div variants={fadeInUp} className="md:col-span-1">
							...
							<div className="flex space-x-3">
								...
								<Image src={'/footer/x.png'} alt="facebook" width={66} height={66} className="h-8 w-8 text-blue-600" priority />
